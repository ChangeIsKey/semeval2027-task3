"""End-to-end pipeline: load data -> run baseline model -> package zip -> run official evaluation."""
import json
...
def run_pipeline(data_root: Path, output_dir: Path, submission_zip: Path):
    ...
    st1_model = BaselineSenseAssigner()
    st2_model = BaselineChangeDetector()
    ...
    for lang in languages:
        ...
        if st2_gold_file.exists():
            ...
            preds_st2 = st2_model.predict(records)
    ...
    print(f'[*] Successfully packaged submission into {submission_zip}')
...
if __name__ == '__main__':
    ...
    work_dir = Path('experiments/baseline_run')
    zip_path = Path('submissions/baseline_submission.zip')
    run_pipeline(mock_dir, work_dir, zip_path)
